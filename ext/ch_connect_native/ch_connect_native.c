...
native_client_recv_step(VALUE self)
{
    ...
    for (;;) {
        ...
        if (pkt.kind == CHC_PKT_DATA) {
            ...
            size_t n_rows = chc_block_n_rows(b);
            size_t n_cols = chc_block_n_columns(b);
            ...
            if (!nc->have_header && n_cols > 0) {
                ...
                for (size_t i = 0; i < n_cols; i++) {
                    ...
                    const chc_type *t = chc_block_column_type(b, i);
                    size_t tlen = 0;
                    ...
                    if (bad) {
                        ...
                        clear_pending(nc);
                        /* connection is sacrificed; the pool reconnects */
                        nc->broken = 1;
            ...
            if (n_rows > 0) {
                ...
                /* decode all columns of this block, then append transposed
                 * rows; col_vals anchors the decoded arrays for GC, cols
                 * gives the transpose loop O(1) access */
...
{
    native_client_t *nc = get_client(self);
    return (!nc->ac || nc->broken || nc->in_flight) ? Qtrue : Qfalse;
}
